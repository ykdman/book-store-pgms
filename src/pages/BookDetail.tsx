...
import { BookDetail as IBookDetail } from "../shared/models";
...
const bookInfoList = [
  ...
  },
  {
    ...
    key: "pub_date",
    filter: (book: IBookDetail) => {
      return formatDate(book.pub_date.split("T")[0]);
    },
  },
  {
    label: "가격",
  ...
  return (
    <BookDetailStyle>
      <header className="header">
        <div className="img">
          <img
            src={getImgSrc(book.img, 500, 600)}
            alt={book.title}
            onClick={() => setIsImgOpen(true)}
          />
        </div>
        <Modal isOpen={isImgOpen} onClose={() => setIsImgOpen(false)}>
          <img src={getImgSrc(book.img, 500, 600)} alt={book.title} />
        </Modal>
      ...
      <div className="content">
        <Tabs>
          <Tab title="상세설명">
            ...
            <ElipsisBox linelimit={2}>{book.detail}</ElipsisBox>
          </Tab>
          <Tab title="목차">
